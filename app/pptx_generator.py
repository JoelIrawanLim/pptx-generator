...
def add_first_song_lyrics_slide(verse_number):
   ...
   if (verse_number + 1 > len(song_1['lyrics'])):
      console.print(
          f"[red bold] An error occured. If you are seeing this, send a bug report. The song is {song_1_id}"
      )
      return 0
...
def add_second_song_lyrics_slide(verse_number):
   ...
   if (verse_number + 1 > len(song_2['lyrics'])):
      console.print(
          f"[red bold] An error occured. If you are seeing this, send a bug report. The song is {song_2_id}"
      )
      return 0
...
def add_third_song_lyrics_slide(verse_number):
   ...
   if (verse_number + 1 > len(song_3['lyrics'])):
      console.print(
          f"[red bold] An error occured. If you are seeing this, send a bug report. The song is {song_3_id}"
      )
      return 0
...
def main(argv):
   ...
   if len(argv) == 4:
      ...
      console.print(
          "[red bold]Not enough arguments provided. Requires at least 3 values"
      )
...
def search(argv):
   ...
   if len(argv) == 3:
      ...
      if argv[2] == "-a":
         ...
         for key in data:
            for index_of_all_authors in range(0, len(data[key]['author'])):
               for index_of_all_words_of_authors in range(
                   0,
                   len(data[key]['author'][index_of_all_authors].split(" "))):
                  for index_of_all_userinput_words in range(
                      0, len(argv[1].split(" "))):
                     if argv[1].split(" ")[index_of_all_userinput_words].lower(
                     ).replace(
                         " ", "").replace("?", "").replace("!", "").replace(
                             ".", ""
                         ) == data[key]['author'][index_of_all_authors].split(
                             " ")[index_of_all_words_of_authors].lower(
                             ).replace(" ", "").replace("?", "").replace(
                                 "!", "").replace(".", ""):
                        already_done = False
                        for done_addresses in range(0, len(addresses)):
                           if addresses[done_addresses] == key:
                              already_done = True
                        if not already_done:
                           table.add_row(key, data[key]['title'],
                                         author_find(key))
         ...
         if found != 1:
            console.print(
                f"There is no author named [red underline bold]{argv[1]}[/] in our song list."
            )
         else:
            console.print(table)
      elif argv[2] == "-t":
         for key in data:
            if argv[1].lower().replace(" ", "").replace("?", "").replace(
                "!",
         ...
         if found != 1:
            console.print(
                f"There is no song named [red underline bold]{argv[1]}[/] in our song list."
            )
         else:
            console.print(table)
      elif argv[2] == "-k":
         if argv[1].isdigit():
            ...
            if found != 1:
               console.print(
                   f"There is no [red underline bold] KRI {argv[1]}[/] in our song list."
               )
            else:
               console.print(table)
         else:
